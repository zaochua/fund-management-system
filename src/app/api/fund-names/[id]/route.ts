...
export async function PUT(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const { rows: existing } = await sql.sql<FundName>`
    ...
    await sql.sql`UPDATE fund_names SET name = ${name}, sector = ${sector} WHERE id = ${id}`;
    
    return NextResponse.json({ id, name, sector, message: '更新成功' });
  } catch (error: unknown) {
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
  try {
    const id = parseInt(params.id);
    await sql.sql`DELETE FROM fund_names WHERE id = ${id}`;
